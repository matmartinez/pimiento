...
import CornerCurve from "./CornerCurve.js"
...
class Renderer {
  ...
  renderTreeFromRootInContext(node, context, offset){
    ...
    if (clips) {
      ...
      const { cornerRadius, cornerCurve } = node
      
      if (cornerRadius <= 0.0) {
        context.rect(offset.x, offset.y, size.width, size.height)
        context.closePath()
      } else {
        this.addCornerRadiusPathInContext(context, cornerRadius, cornerCurve, Rect.make(offset.x, offset.y, size.width, size.height))
      }
  ...
  addCornerRadiusPathInContext(context, cornerRadius, cornerCurve, frame){
    let createCornerRadiusFunction
        
    if (cornerCurve === CornerCurve.circular) {
      createCornerRadiusFunction = ContextFunctions(context).createRoundedRectPath
      
    } else if (cornerCurve === CornerCurve.continuous) {
      createCornerRadiusFunction = ContextFunctions(context).createSquirclePath
      
    } else {
      throw `Encountered unknown corner curve type: "${cornerCurve}"`
    }
    
    createCornerRadiusFunction(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height, cornerRadius)
  }
